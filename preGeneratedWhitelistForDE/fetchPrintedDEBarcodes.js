 ...
 * this will only fetch the tickets which type not in online
...
async function main() {
  ...
  try {
    ...
    const res = await Promise.map(
      ...
      async (itemGroup) => {
        ...
        const barcodes = await Barcodes.find({
          inventory_id: { $in: _.map(invs, "_id") },
          item_id: { $in: _.map(items, ObjectId) },
          active: true,
        }).toArray();
        ...
        _.map(barcodes, (barcode) => {
          ...
          if (barcode.discount_type == "complimentary") {
            ...
            //     "Stand Name": name
            // }
            // whitelistDump.push(scannerQRcodeData);
          } else {
            const scannerData = {
              event_id: barcode.event_id.toString(),
              item_id: barcode.item_id.toString(),
        ...
        // also fetch pregenerated whitelists
        const preGeneratedWhitelists = await Whitelists.find({
          core_item_id: { $in: _.map(items, ObjectId) },
          is_used: false,
          pregenerated: true,
        }).toArray();

        // console.log({ preGeneratedWhitelists: preGeneratedWhitelists.length });

        _.map(preGeneratedWhitelists, (whitelist) => {
          let barcodeString = whitelist.barcode;
          const scannerData = {
            event_id: eventId,
            item_id: whitelist.core_item_id.toString(),
            item_name: name,
            sector: "01",
            barcode: barcodeString,
            "Is Used": 1,
            "Is Active": 1,
            shortcode: "NA", // pregenerated
            "Is Blacklisted": 0,
            "Stand Name": name,
          };
          whitelistDump.push(scannerData);
    ...
    if (whitelistDump.length > 0) {
      ...
      fs.writeFileSync(
        `./whitelist_exports/csk_physical_${match_no}_${new Date().toISOString()}.csv`,
        rowsData
      );
    }
