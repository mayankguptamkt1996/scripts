 ...
 * This is a hack for Zomaland Pune 2022
...
const eventId = '6780a78e3eedc6f6ada93272';
...
async function main() {
    ...
    try {
        ...
        const res = await Promise.map(_.values(itemsMap), async (itemGroup) => {
            ...
            const barcodesOnline = await Barcodes.find({
                inventory_id: { $in: _.map(onlineInvs, "_id") },
                item_id: { $in: _.map(items, ObjectId) },
                active: true,
            }).toArray();

            _.map(barcodesOnline, (barcode) => {
                const { vendorDetails, shortcode } = barcode.payload;
                const foundInventory = onlineInvs.find(inventory => inventory.shortcode === shortcode);
                ...
                const scannerData = {
                    ...
                    "Is Blacklisted": 0,
                    "Stand Name": name,
                    "delivery_email":foundInventory.delivery_email
                };
            ...
            _.map(barcodes, (barcode) => {
                const { vendorDetails, shortcode } = barcode.payload;
                const foundInventory = invs.find(inventory => inventory.shortcode === shortcode);
                let barcodeString = barcode.barcode;
                const scannerData = {
                    event_id: barcode.event_id.toString(),
                    item_id: barcode.item_id.toString(),
        ...
        if (whitelistDump.length > 0) {
            ...
            fs.writeFileSync(`./whitelist_exports/fc_${new Date().toISOString()}.csv`, rowsData);
        }
    } catch (e) {
        timeStamp({ e });
    } finally {
