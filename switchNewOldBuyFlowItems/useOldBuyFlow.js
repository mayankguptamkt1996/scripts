...
const main = async () => {
  ...
  console.log("STARTING SWITCH TO OLD BUY FLOW for ENV =", ENV);
  ...
  try {
    ...
    await Promise.map(csvData, async (csvRow) => {
      ...
      const itemId = csvRow.itemId
        try {
          await items.updateOne({ _id: ObjectId(itemId) }, { $set: { addon_items: oldBuyFlowAddOn.map(i=> ObjectId(i)) } });
          console.log(`Updated add ons for ${itemId} to OLD Buy Flow: ` + JSON.stringify(oldBuyFlowAddOn));
          responseCsv.push({
    ...
    fs.writeFileSync(`./old-success-response-${new Date().getTime()}.csv`, responseCsvFinal);

    // write the error CSV file
    const errorCsvOptions = { headers: "keys" };
    const errorCsvFinal = csvjson.toCSV(errorCsv, errorCsvOptions);
    fs.writeFileSync(`./old-error-response-${new Date().getTime()}.csv`, errorCsvFinal);
