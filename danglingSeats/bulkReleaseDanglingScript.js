...
const connectionString = {
  prod:
    ...
    "@rs0-shard-00-00-runtd.mongodb.net:27017,rs0-shard-00-01-runtd.mongodb.net:27017,rs0-shard-00-02-runtd.mongodb.net:27017/insidercore?ssl=true&replicaSet=rs0-shard-0&authSource=admin&readPreference=secondaryPreferred",
  stage:
    "mongodb://" +
    process.env.MONGODB_USERNAME +
    ":" +
    process.env.MONGODB_PASSWORD +
    "@db1.stage.insider.in:27017?authSource=admin",
};
...
const bulkReleaseCall = (seatsIoEventId, seats) => {
  if (_.isEmpty(seats)) {
    console.log("No Seats to release!");
    return;
  }

  console.log(`Releasing ${seats.length} seats from Seats.io`);
  return callSeatsIo({
    url: `https://api-${SEATS_IO_API_REGION}.seatsio.net/events/${seatsIoEventId}/actions/release`,
    body: { objects: seats, orderId: `Dangling script (release)` },
  });
};
...
const main = async () => {
  ...
  const seatsIoId = prompt(
    "Please enter the seatsIoId (eg: 774d4e5c-6c3a-4689-bfa8-d2507b21b3ca): "
  ).trim();
  const allow = prompt(
  ...
  await Promise.map(shows, async (show) => {
    try {
      const itemgroups = show.items_for_sale.filter(itemGroup => seatsIoId === '' || itemGroup.seats_io_id === seatsIoId);
      await Promise.map(
        itemgroups,
        async (itemGroup) => {
          ...
          if (!_.isEmpty(key)) {
            ...
            console.log('For seatsIoId: ', key);
            console.log('seatsInMongo: ', seatsInMongo);
            console.log('seatsInSeatIo: ', seatsInSeatIo);
            console.log('seatsInRedis: ', seatsInRedis);
            console.log('blockedInDb: ', blockedInDb);
            console.log('toBeReleased: ', toBeReleased);
            console.log('toBeBlocked: ', toBeBlocked);
            releaseCount = releaseCount + toBeReleased.length

            // console.log(`[${key}] Checking Blocking...`);
            // await Promise.map(
            //   toBeBlocked,
            //   async (seat) => {
            ...
            //       item_id: { $in: itemArr },
            //       status: {
            //         $nin: ["insured_cancelled", "refunded", "cancelled"],
            //       },
            //     });
            ...
            let seatsToRelease = [];
            console.log(`[${key}] Checking Releasing...`);
            await Promise.map(
              toBeReleased,
              async (seat) => {
                const inv = await Sold.findOne({
                ...
                if (_.isEmpty(inv)) {
                  ...
                  console.log(`Inventory exists for ${seat}`);
                  return Promise.resolve();
                }
              },
              { concurrency: 1 }
            ...
            if (seatsToRelease.length) {
              const bulkRelease = await bulkReleaseCall(key, seatsToRelease);

              if (bulkRelease.status !== 204) {
                console.log(`Error in releasing seats for ${key}`);
                console.log(JSON.stringify(seatsInMongo, null, 4));
                return;
              } else {
                console.log(`Seats released successfully for ${key}`);
              }
            } else {
              console.log('No seats to release!')
            }
...
main()
  .then((result) => {
    console.log(result);
    console.log(`Total Seats released ${releaseCount}`);
    process.exit(0);
  })
